...
class EMData extends DeviceComponent {
  ...
  async *getPeriodIterator(startTS, endTS, incPeriod = 1, respectUserPeriod = false) {
    ...
    do {
      ...
      for (const _dataBlock of _emDataResult.response.data) {
        // Only override incPeriod if respectUserPeriod is false (default behavior)
        if (!respectUserPeriod) {
        ...
        for (const _dataRow of _dataBlock.values) {
          ...
          yield {
            ...
            actualPeriod: _dataBlock.period, // Show actual device period
            requestedPeriod: incPeriod, // Show requested period
          };
        }
      }
